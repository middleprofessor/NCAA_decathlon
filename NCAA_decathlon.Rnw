...
plot_old_fig_2 <- function(){
  # this fig is now in the supplement
...
\newline
\textbf{Fig. 1} The distribution of the raw, Pearson product-moment correlations of the A) NCAA data and B) simulated data.
...
This section re-analyzes the soccer athletic performance data \citep{wilson_does_2014} using Wright's path-analytic factor analysis with bias correction to estimate quality-free correlations \citep{wright_general_1932}. Both partial correlations conditional on all other performance variables \citep{van_damme_performance_2002} and regression residual correlations \citep{wilson_does_2014} were also used to estimate the quality-free correlations. Pearson, Partial (PCOR), Regression Residual (RR), and Wright's bias corrected (WBC) correlations are given in Table 1. The error intervals associated with each method are illustrated in Fig. 2 and were computed using Monte Carlo simulation of 1000 data sets with a distribution of correlations similar to that of the soccer data (see Methods) The Pearson correlations are from Table 1 of \cite{wilson_does_2014}. The regression residual correlations are from Table 2 of \cite{wilson_does_2014}. The WBC correlations were computed using the PC1 loadings in Table 3 of \cite{wilson_does_2014} (these loadings are estimates of the Quality path coefficients $\alpha$).
...
\begin{tabular}{rllrrrr}
  \hline
 & Event\_A & Event\_B & Pearson & PCOR & RR & WBC \\ 
  \hline
...
do_wilson <- function(){
  ...
  plot_wilson_errors(error_table,fig_no='01')
...
\includegraphics[scale=.65]{SuppFig01.pdf}
\newline
...
<<Wilson Table,echo=FALSE>>=
  ...
  R_table_form <- R_table[,list(Event_A,Event_B,Pearson=round(Pearson,2),PCOR=round(Pcor,2),RR=round(RR,2),WBC=round(WGSBC,2))]
