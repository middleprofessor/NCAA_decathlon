...
\begin{document}
\title{R Scripts used in ``Performance trade-offs and Individual Quality in NCAA decathletes"}
\author{Jeffrey A. Walker\thanks{Department of Biological Sciences, University of Southern Maine, 70 Falmouth St., Portland, ME, 04103, U.S.A., E-mail: walker@maine.edu}}

\maketitle

\begin{flushleft}

<<housekeeping,echo=FALSE,message=FALSE,warning=FALSE,prompt=FALSE>>=
library(data.table)
library(ggplot2)
...
\section{Path Models of Functional Trade-offs masked by Quality}
...
delta_s_11*delta_s_12 + delta_s_21*delta_s_22 + 
  delta_s_11*chi_s_1*chi_s_2*delta_s_22 + delta_s_12*chi_s_1*chi_s_2*delta_s_21
...
# the 3rd and 4th parts use delta instead of alpha because the correlation
# between M_1 and M_2 requires all of the variance and not just that due to Q
...
r_alpha
# computing r_alpha without the standardized coefficients shows that sd_m1 and 
# sd_m2 are in both num and denom and cancel
# = r_alpha
...
\subsection{How quality or a facilitation does not mask trade-offs at the whole animal level}
This is a script to explore the section "What facilitations or quality do not mask" in the Discussion of the paper.
\begin{equation} \label{eq:discussion_a}
  \begin{tikzpicture}
  	\node (1) {$M_1$};
		\node[below right=of 1] (2) {$P_1$};
		\node[right=of 2] (3) {$P_2$};
		\node[above right=of 3] (4) {$Q$};
		\draw [->] (1) -- node[left,font=\footnotesize]{-.4} (2);
		\draw [->] (1) -- node[above,font=\footnotesize]{.4} (3);
		\draw [->] (4) -- node[above,font=\footnotesize]{.7} (2);
		\draw [->] (4) -- node[right,font=\footnotesize]{.7} (3);
	\end{tikzpicture}
 \end{equation}

<< How quality or a facilitation does not mask trade-offs at the whole animal level>>=
  ...
  E_1 <- rnorm(n)
  E_2 <- rnorm(n)
  P_1 <- alpha_1*Q + beta_1*M + sqrt(1 - alpha_1^2 - beta_1^2)*E_1
  P_2 <- alpha_2*Q + beta_2*M + sqrt(1 - alpha_2^2 - beta_2^2)*E_2
  alpha_1*alpha_2 + beta_1*beta_2 # expected r
  cor(P_1,P_2) # actual r
  
  # add training. Two notes
  # 1) The response of the M trait to training is precisely the same. There will obviously 
  # be variation in real humans
  # 2) The Error in the performance is precisely that before training (that is the error 
  # has correlation = 1), which means that an athlete that was .37 units above the mean in 
  # P_1 prior to training was also .37 units above the mean post training. In reality this 
  # error will likely be correlated (even high correlated) but not 1. Even if the error 
  # post training is independent of the error pre-training, the expected mean response is   
  # delta_M*beta
  delta_M <- .5
  M_trained <- M + delta_M
  E1_post <- E_1 # make this rnorm(n) to make it independent of E_1
  E2_post <- E_2
  P1_trained <- alpha_1*Q + beta_1*M_trained + sqrt(1 - alpha_1^2 - beta_1^2)*E1_post
  P2_trained <- alpha_2*Q + beta_2*M_trained + sqrt(1 - alpha_2^2 - beta_2^2)*E2_post
  delta_P1 <- (P1_trained - P_1)
  delta_P2 <- (P2_trained - P_2)
  beta_1*delta_M # expected mean response in P1
  beta_2*delta_M #expected mean response in P2
  mean(delta_P1) # sample mean response to training for P1
  mean(delta_P2) # sample mean response to training for P2
  data.frame(delta_P1=delta_P1,delta_P2=delta_P2)[1:5,]
@
...
do_it <- function(){
  ...
  # do repeatability (intraclass-correlation)
  clean_data <- data.table(read.table('ncaa.2014.clean.txt',header=TRUE,sep='\t',colClasses=c(rep('factor',5),rep('numeric',10))))
  .sdcols <- c('run_100m','run_400m','run_1500m','run_110h','high_jump','long_jump','pole_vault','shot_put','discus','javelin')
  # this is terrible code because it allows errors to creep in if the order of columns change
  icc_res <- numeric(10)
  icc_res[1] <- unlist(ICCbare(Name,run_110h,data=clean_data))
  icc_res[2] <- unlist(ICCbare(Name,run_400m,data=clean_data))
  icc_res[3] <- unlist(ICCbare(Name,run_1500m,data=clean_data))
  icc_res[4] <- unlist(ICCbare(Name,run_400m,data=clean_data))
  icc_res[5] <- unlist(ICCbare(Name,high_jump,data=clean_data))
  icc_res[6] <- unlist(ICCbare(Name,long_jump,data=clean_data))
  icc_res[7] <- unlist(ICCbare(Name,pole_vault,data=clean_data))
  icc_res[8] <- unlist(ICCbare(Name,shot_put,data=clean_data))
  icc_res[9] <- unlist(ICCbare(Name,discus,data=clean_data))
  icc_res[10] <- unlist(ICCbare(Name,javelin,data=clean_data))
  icc <- data.table(event=.sdcols,icc=icc_res)
...
#    hist(event_data[,which_event],xlab=which_event,main=which_event)
 ...
 t(cor_data[,1])%*%cor_data[,2]/sqrt(t(cor_data[,1])%*%cor_data[,1])/sqrt(t(cor_data[,2])%*%cor_data[,2])
  ...
  colnames(R_raw_tile) <- c('Event_A', 'Event_B', 'Pearson')
  diag(R_pcor) <- 1.0
  R_pcor_tile <- melt(R_pcor)
  colnames(R_pcor_tile) <- c('Event_A', 'Event_B', 'Pcor')
  diag(R_cor_rr) <- 1.0
  R_cor_rr_tile <- melt(R_cor_rr)
  colnames(R_cor_rr_tile) <- c('Event_A', 'Event_B', 'RR')
  diag(R_wgsuc) <- 1.0
  R_wgs_uc_tile <- melt(R_wgsuc)
  colnames(R_wgs_uc_tile) <- c('Event_A', 'Event_B', 'WGSUC')
  diag(R_wgsbc) <- 1.0
  R_wgs_bc_tile <- melt(R_wgsbc)
  colnames(R_wgs_bc_tile) <- c('Event_A', 'Event_B', 'WGSBC')
  R_quality_tile <- melt(R_quality)
  colnames(R_quality_tile) <- c('Event_A', 'Event_B', 'R_q')
  
  R_tile <- merge(R_raw_tile,R_pcor_tile,by=c('Event_A','Event_B'))
  R_tile <- merge(R_tile,R_cor_rr_tile,by=c('Event_A','Event_B'))
  R_tile <- merge(R_tile,R_wgs_uc_tile,by=c('Event_A','Event_B'))
  R_tile <- merge(R_tile,R_wgs_bc_tile,by=c('Event_A','Event_B'))
  R_tile <- merge(R_tile,R_quality_tile,by=c('Event_A','Event_B'))
  R_tile <- data.table(R_tile)
  R_tile[,Event_A:=reorder_event_levels(Event_A)]
  R_tile[,Event_B:=reorder_event_levels(Event_B)]
  
  plot_fig_2(R_tile)
  plot_fig_3(R_tile,axis_labels=c('D','SP','J','HJ','LJ','PV','110','100','400','1500'),fig_no='03')
  ...
  plot_fig_4(error_table,quant_table, fig_no='04')
...
gwfa <- function(X,which_size='pc1',fit,bc='none',b=0){ # generalized wright factor analysis
  ...
  X_s <- standardize(X) # how does not mean centering effect alpha_hat=loadings?
...
simulate_data <- function(data,wgs_fit,btable,niter=100){
  ...
  M_fit <- fit_factor_matrix(R_resid,bc_alpha) # fit for size-free data
  ...
  for(iter in 1:niter){
    ...
    # get size-free R
    # original code had estimate of size-free Y and cor(Y) to get error due to sampling only
...
theme_jeb <- theme_bw() + theme(text=element_text(family = "Arial"), plot.margin=unit(x=c(0,0.1,0,0),'cm'), plot.title=element_text(hjust=0), axis.title.x = element_blank(), axis.text=element_text(size=8), axis.title.y=element_blank(),legend.position=c('bottom'),legend.key.width=unit(0.4, "in"))
...
plot_fig_2 <- function(R_tile){
  # the correlation heat-map for the Pearson correlations
  # max w: 180 mm = 7.08661 inch
  # max h: 210 mm = 8.26772 inch
  # 12 pt bold for figure panels
  # 8 pt Arial for labels
  gg1 <- ggplot(data=R_tile,aes(x=Event_A,y=Event_B,fill=Pearson))
  gg1 <- gg1 + geom_tile()
  gg1 <- gg1 + geom_text(aes(Event_B, Event_A, label = round(Pearson,2)), color = "black", size = 2.1) # 2.1 = 6 pt
  gg1 <- gg1 + scale_fill_gradient2(limits=c(-1, 1),name='Cor')
  ...
  gg1 <- gg1 + theme_bw()
  gg1 <- gg1 + theme(text=element_text(family = "Arial"),plot.margin=unit(x=c(0,0.1,0,0),'cm'),plot.title=element_text(hjust=0,size=12),axis.title.x = element_blank(),axis.text=element_text(size=8), axis.title.y=element_blank(),legend.text=element_text(size=8),legend.position=c('bottom'),legend.key.width=unit(0.4, "in"))
#  gg1 <- gg1 + theme_jeb()
  gg1 <- gg1 + guides(fill = guide_colorbar(ticks = FALSE))

#  postscript('Fig02.eps',horizontal=FALSE,onefile=FALSE,paper='special',height=4.0,width=3.25)
 # print(gg1)
#  dev.off()
  
  pdf('Fig02.pdf',paper='special',onefile=FALSE,width=3.25,height=4)
  showtext.begin()
  print(gg1)
  showtext.end()
  dev.off()
...
plot_fig_3 <- function(R_tile,axis_labels,fig_no){
  # the correlation heat-map plot of the four different adjustments
  gg1 <- ggplot(data=R_tile,aes(x=Event_A,y=Event_B,fill=Pcor))
  gg1 <- gg1 + geom_tile()
  gg1 <- gg1 + geom_text(aes(Event_B, Event_A, label = round(Pcor,2)), color = "black", size = 2.1)
  ...
  gg1 <- gg1 + theme_bw()
  gg1 <- gg1 + theme(text=element_text(family = "Arial"),plot.margin=unit(x=c(0,.1,0,0),'cm'),plot.title=element_text(hjust=0,size=12),axis.title.x = element_blank(),axis.text=element_text(size=8), axis.title.y=element_blank(),legend.position=c('none'))
  
  gg2 <- ggplot(data=R_tile,aes(x=Event_A,y=Event_B,fill=RR))
  gg2 <- gg2 + geom_tile()
  gg2 <- gg2 + geom_text(aes(Event_B, Event_A, label = round(RR,2)), color = "black", size = 2.1)
  ...
  gg2 <- gg2 + theme_bw()
  gg2 <- gg2 + theme(text=element_text(family = "Arial"),plot.margin=unit(x=c(0,.1,0,0),'cm'),plot.title=element_text(hjust=0,size=12),axis.title.x = element_blank(),axis.text=element_text(size=8), axis.title.y=element_blank(),legend.position=c('none'))
  
  gg3 <- ggplot(data=R_tile,aes(x=Event_A,y=Event_B,fill=WGSUC))
  gg3 <- gg3 + geom_tile()
  gg3 <- gg3 + geom_text(aes(Event_B, Event_A, label = round(WGSUC,2)), color = "black", size = 2.1)
  ...
  gg3 <- gg3 + ggtitle(expression(paste('C  ',italic('Wright Uncorrected'))))
  gg3 <- gg3 + theme_bw()
  gg3 <- gg3 + theme(text=element_text(family = "Arial"),plot.margin=unit(x=c(0,.1,0,0),'cm'),plot.title=element_text(hjust=0,size=12),axis.title.x = element_blank(),axis.text=element_text(size=8), axis.title.y=element_blank(),legend.position=c('none'))
  
  gg4 <- ggplot(data=R_tile,aes(x=Event_A,y=Event_B,fill=WGSBC))
  gg4 <- gg4 + geom_tile()
  gg4 <- gg4 + geom_text(aes(Event_B, Event_A, label = round(WGSBC,2)), color = "black", size = 2.1)
  ...
  gg4 <- gg4 + ggtitle(expression(paste('D  ',italic('Wright Bias-Corrected'))))
  gg4 <- gg4 + theme_bw()
  gg4 <- gg4 + theme(text=element_text(family = "Arial"),plot.margin=unit(x=c(0,.1,0,0),'cm'),plot.title=element_text(hjust=0,size=12),axis.title.x = element_blank(),axis.text=element_text(size=8), axis.title.y=element_blank(),legend.position=c('none'))
...
#  dev.off()
  ...
  pdf(fig_name,paper='special',onefile=FALSE,width=6.5,height=6.5)
  showtext.begin()
  print(gg1)
  print(gg2)
  print(gg3)
...
plot_fig_4 <- function(error_table,quant_table,fig_no){ # the error interval plot
  # explored plotting median and interval widths on the plot which uses this code
  error_table <- reorder_method_levels(error_table)
  meds <- error_table[method!='Pearson',list(med=median(error),high=quantile(error,.975),low=quantile(error,.025)),by=method]
  meds[,CI_width:=high-low]
  
  # checking to see if the error intervals are well described by the mean and standard deviation. They are.
  psd <- 68.2689492 # percent within 1SD of mean
  psd2 <- 95.4499736 # percent within 2SD of mean
  up1 <- .5 + psd/2/100
  up2 <- .5 + psd2/2/100
  lo1 <- .5 - psd/2/100
  lo2 <- .5 - psd2/2/100
  sum_table <- error_table[method!='Pearson',list(med=median(error),up1=quantile(error,up1),up2=quantile(error,up2),lo1=quantile(error,lo1),lo2=quantile(error,lo2),sd=sd(error)),by=method]
  sum_table[,samp2sd:=up2-lo2]
  sum_table[,para2sd:=4*sd]
  sum_table[,samp1sd:=up1-lo1]
  sum_table[,para1sd:=2*sd]
  # just report mean and SD
  sum_table <- error_table[,list(mean=round(mean(error),3),sd=round(sd(error),3)),by=method]

  
  gg1 <- ggplot(data=error_table[method!='Pearson',],aes(x=method,y=error))
#  gg1 <- gg1 + stat_summary(fun.data=boxplot.95,geom='boxplot',aes(fill=method),position='dodge')
  gg1 <- gg1 + stat_summary(fun.data=boxplot.95,geom='boxplot',position='dodge')
  gg1 <- gg1 + scale_y_continuous(name='Error')
  gg1 <- gg1 + scale_x_discrete(name='Method',labels=c("pcor" = "PCOR", "rr" = "RR",
                              "wgsuc" = "WUC", "wgsbc" = "WBC"))
  gg1 <- gg1 + ggtitle('A')
  gg1 <- gg1 + theme_bw()
  gg1 <- gg1 + theme(aspect.ratio=1, text=element_text(family = "Arial"),plot.margin=unit(x=c(0,.1,0,0),'cm'),axis.title=element_text(size=8),axis.text=element_text(size=8),plot.title=element_text(hjust=0,size=12),strip.text=element_text(size=8),legend.title=element_blank(),legend.position=c('none'))
  ...
  gg2 <- gg2 + ggtitle('B')
  gg2 <- gg2 + theme_bw()
  gg2 <- gg2 + theme(aspect.ratio=1,text=element_text(family = "Arial"),plot.margin=unit(x=c(0,.1,0,0),'cm'),axis.title=element_text(size=8),axis.text=element_text(size=8),plot.title=element_text(hjust=0,size=12),strip.text=element_text(size=8),legend.title=element_blank(),legend.position=c('none'))
...
<<Supp Fig 01: Distribution of Correlations, echo=FALSE,message=FALSE>>=
...
gg1 <- gg1 + theme(text=element_text(family = "Arial"),plot.margin=unit(x=c(0,.35,0,0),'cm'),axis.title=element_text(size=11),axis.text=element_text(size=9),plot.title=element_text(hjust=0),strip.text=element_text(size=10),legend.title=element_blank(),legend.text=element_text(size=9),legend.position=c('none'),legend.margin=unit(-0.6,"cm"))
...
gg2 <- gg2 + theme(text=element_text(family = "Arial"),plot.margin=unit(x=c(0,.35,0,0),'cm'),axis.title=element_text(size=11),axis.text=element_text(size=9),plot.title=element_text(hjust=0),strip.text=element_text(size=10),legend.title=element_blank(),legend.text=element_text(size=9),legend.position=c('none'),legend.margin=unit(-0.6,"cm"))


  fig_name <- 'SupFig01.pdf'
  w <- 6
  h <- 3.25
  pdf(fig_name,paper='special',onefile=FALSE,width=w,height=h)
  showtext.begin()
...
do_wilson <- function(){
  ...
  colnames(R_cor_tile) <- c('Event_A', 'Event_B', 'Pearson')
  diag(R_cor_pcor) <- 1.0
  R_cor_pcor_tile <- melt(R_cor_pcor)
  colnames(R_cor_pcor_tile) <- c('Event_A', 'Event_B', 'Pcor')
  diag(R_cor_rr) <- 1.0
  R_cor_rr_tile <- melt(R_cor_rr)
  colnames(R_cor_rr_tile) <- c('Event_A', 'Event_B', 'RR')
  diag(R_wgsbc) <- 1.0
  R_wgs_tile <- melt(R_wgsbc)
  colnames(R_wgs_tile) <- c('Event_A', 'Event_B', 'WGSBC')
  
  R_tile <- merge(R_cor_tile,R_cor_pcor_tile,by=c('Event_A','Event_B'))
  R_tile <- merge(R_tile,R_cor_rr_tile,by=c('Event_A','Event_B'))
  R_tile <- merge(R_tile,R_wgs_tile,by=c('Event_A','Event_B'))
  colnames(R_tile)[1:2] <- c('Event_A','Event_B')
  R_tile <- data.table(R_tile)
  
  plot_wilson_correlations(R_tile,axis_labels=c('1500','squat','jump','sprint','agility'),fig_no='05')
  ...
  plot_wilson_errors(error_table)
...
plot_wilson_correlations <- function(R_tile,axis_labels,fig_no){
  ...
  gg1 <- ggplot(data=R_tile,aes(x=Event_A,y=Event_B,fill=Pearson))
  gg1 <- gg1 + geom_tile()
  gg1 <- gg1 + geom_text(aes(Event_B, Event_A, label = round(Pearson,2)), color = "black", size = 2.1)
  gg1 <- gg1 + scale_fill_gradient2(limits=c(-1, 1))
  gg1 <- gg1 + scale_x_discrete(labels=axis_labels)
  gg1 <- gg1 + scale_y_discrete(labels=axis_labels)
  ...
  gg2 <- gg2 + geom_tile()
  gg2 <- gg2 + geom_text(aes(Event_B, Event_A, label = round(Pcor,2)), color = "black", size = 2.1)
  gg2 <- gg2 + scale_fill_gradient2(limits=c(-1, 1))
  gg2 <- gg2 + scale_x_discrete(labels=axis_labels)
  gg2 <- gg2 + scale_y_discrete(labels=axis_labels)
  gg2 <- gg2 + ggtitle(expression(paste('B  ',italic('Partial Correlations'))))
  gg2 <- gg2 + theme_bw()
  gg2 <- gg2 + theme(text=element_text(family = "Arial"),plot.margin=unit(x=c(0,0.1,0,0),'cm'),plot.title=element_text(hjust=0,size=12),axis.title.x = element_blank(),axis.text=element_text(size=8), axis.title.y=element_blank(),legend.position=c('none'))
  
  gg3 <- ggplot(data=R_tile,aes(x=Event_A,y=Event_B,fill=RR))
  gg3 <- gg3 + geom_tile()
  gg3 <- gg3 + geom_text(aes(Event_B, Event_A, label = round(RR,2)), color = "black", size = 2.1)
  gg3 <- gg3 + scale_fill_gradient2(limits=c(-1, 1))
  gg3 <- gg3 + scale_x_discrete(labels=axis_labels)
  gg3 <- gg3 + scale_y_discrete(labels=axis_labels)
  gg3 <- gg3 + ggtitle(expression(paste('C  ',italic('PC1 Regression Residuals'))))
  gg3 <- gg3 + theme_bw()
  gg3 <- gg3 + theme(text=element_text(family = "Arial"),plot.margin=unit(x=c(0,0.1,0,0),'cm'),plot.title=element_text(hjust=0,size=12),axis.title.x = element_blank(),axis.text=element_text(size=8), axis.title.y=element_blank(),legend.position=c('none'))
  ...
  gg4 <- gg4 + ggtitle(expression(paste('D  ',italic('Wright Bias-Corrected'))))
  gg4 <- gg4 + theme_bw()
  gg4 <- gg4 + theme(text=element_text(family = "Arial"),plot.margin=unit(x=c(0,0.1,0,0),'cm'),plot.title=element_text(hjust=0,size=12),axis.title.x = element_blank(),axis.text=element_text(size=8), axis.title.y=element_blank(),legend.position=c('none'))
...
plot_wilson_errors <- function(error_table){ # the error interval plot
  error_table <- reorder_method_levels(error_table)
  gg1 <- ggplot(data=error_table[method!='samp',],aes(x=method,y=error))
  gg1 <- gg1 + stat_summary(fun.data=boxplot.95,geom='boxplot',position='dodge')
  gg1 <- gg1 + scale_y_continuous(name='Error')
  gg1 <- gg1 + scale_x_discrete(labels=c("pcor" = "PCOR", "rr" = "RR",
                              "wgsuc" = "WUC", "wgsbc" = "WBC"))
  gg1 <- gg1 + theme_bw()
  gg1 <- gg1 + theme(text=element_text(family = "Arial"),plot.margin=unit(x=c(0,0.1,0,0),'cm'),axis.title=element_text(size=11),axis.text=element_text(size=9),plot.title=element_text(hjust=0),strip.text=element_text(size=10),legend.title=element_blank(),legend.text=element_text(size=9),legend.position=c('none'),legend.margin=unit(-0.6,"cm"))
  gg1

  fig_name <- 'SuppFig02.pdf'
  pdf(fig_name,paper='special',onefile=FALSE,width=3.25,height=3.25)
  showtext.begin()
  print(gg1)
  showtext.end()
  dev.off()
